"""
Created on Tue Jun 14 10:45:56 2022
...
for seconds in range(200):
    ...
    df2['S_0_new'] = (df2['S_0'] * np.random.choice(range(0,10)))/10
    df2['S_1_new'] = (df2['S_1'] * np.random.choice(range(0,10)))/10

    # creating KPIs
    avg_age = np.mean(df['age_new'])
    avg_S0 = np.mean(df2['S_0_new'])

    count_married = int(df[(df["marital"] == 'married')]['marital'].count() + np.random.choice(range(1, 30)))
    count_S_2 = int(df2[(df2["S_2"] == 'S_2')]['S_2'].count() + np.random.choice(range(1,30)))
    ...
    with placeholder.container():
        ...
        kpi11, kpi22, kpi33 = st.columns(3)
            ...
            # fill in those three columns with respective metrics or KPIs
        kpi11.metric(label="Age ⏳", value=round(avg_age), delta=round(avg_age) - 10)
        kpi22.metric(label="Married Count 💍", value=int(count_married), delta=- 10 + count_married)
        kpi33.metric(label="A/C Balance ＄", value=f"$ {round(balance, 2)} ",
                    delta=- round(balance / count_married) * 100)
